...
def acq_max(ac, gp, y_max, bounds, iteration, gp_samples=None):
    print("[Running the direct optimizer]")
    bound_list = []
    for b in bounds:
        bound_list.append(tuple(b))

    res = mini_direct(ac, bound_list, para_dict={"gp":gp, "y_max":y_max, "iteration":iteration, "gp_samples":gp_samples})
    x_max = res["x"]
...
class UtilityFunction(object):
    ...
    def utility(self, x, para_dict):
        gp, y_max, iteration, gp_samples = para_dict["gp"], para_dict["y_max"], para_dict["iteration"], para_dict["gp_samples"]
        
        if self.kind == 'ucb':
            return self._ucb(x, gp, self.kappa, self.use_fixed_kappa, self.kappa_scale, iteration, self.gp_model, gp_samples)
        if self.kind == 'ei':
    ...
    @staticmethod
    def _ei(x, gp, y_max, xi, gp_model, gp_samples):
        if USE_DIRECT_OPTIMIZER:
            x = x.reshape(1, -1)
