...
from bayesian_optimization_taf import BayesianOptimization_meta
...
meta_x = [meta_x[1], meta_x[2], meta_x[3]]
meta_y = [meta_y[1], meta_y[2], meta_y[3]]
...
for itr in run_list:
    BO_taf = BayesianOptimization_meta(f=obj_func_cifar,
            pbounds={'lr':(0, 1), 'lr_decay':(0, 1), 'l2':(0, 1)}, gp_opt_schedule=5, \
            gp_model='gpy', use_init="initializations/cifar_10_init_" + str(itr) + ".p", gp_mcmc=False, \
            log_file="results/cifar_10_taf_rho_05_" + str(itr) + ".p", save_init=False, \
            save_init_file=None, meta_functions=meta_functions, ARD=True, kernel_rho=0.5)
    BO_taf.maximize(n_iter=25, init_points=5, kappa=3, use_fixed_kappa=False, kappa_scale=0.3, acq='ei')
