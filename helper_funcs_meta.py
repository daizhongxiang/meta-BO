...
def acq_max(ac, gp, meta_gps, w, tau, eta, y_max, bounds, iteration, gp_samples=None):
    print("[Running the direct optimizer]")
    bound_list = []
    for b in bounds:
        bound_list.append(tuple(b))

    res = mini_direct(ac, bound_list, para_dict={"gp":gp, "meta_gps":meta_gps, "w":w, "tau":tau, "eta":eta, "y_max":y_max, "iteration":iteration, "gp_samples":gp_samples})
    x_max = res["x"]
...
class UtilityFunction(object):
    ...
    def utility(self, x, para_dict):
        gp, meta_gps, w, tau, eta, y_max, iteration, gp_samples = para_dict["gp"], para_dict["meta_gps"], para_dict["w"], \
        ...
        if self.kind == 'ei':
            return self._ei(x, gp, y_max, self.xi, self.gp_model, gp_samples)
        if self.kind == 'poi':
    ...
    def _ucb(x, gp, meta_gps, w, tau, eta, kappa, use_fixed_kappa, kappa_scale, iteration, gp_model, gp_samples):
        ...
        if gp_model == 'sklearn':
            ...
            for i in range(M):
                ...
                ucb_1 += ((mean_m + np.sqrt(tau)*std_m) * w[i])

            if not (gp is None):
                mean, var = gp.predict(x)
                std = np.sqrt(var)            
                if use_fixed_kappa:
                    ucb_2 = mean + kappa * std
                else:
                    ucb_2 = mean + np.sqrt(kappa_scale * d * np.log(2 * iteration)) * std
            else:
                ucb_2 = 0

            ucb_overall = eta * ucb_1 + (1 - eta) * ucb_2
    ...
    @staticmethod
    def _ei(x, gp, y_max, xi, gp_model):
