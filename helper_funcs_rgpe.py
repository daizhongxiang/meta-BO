...
def acq_max(ac, gp, meta_gps, w, tau, eta, y_max, bounds, iteration, gp_samples=None, data_mask=None):
...
class UtilityFunction(object):
    ...
    def utility(self, x, para_dict):
        ...
        if self.kind == 'ei':
            return self._ei(x, gp, y_max, self.xi, self.gp_model, gp_samples, meta_gps, w)
        if self.kind == 'poi':
...
#         optimizer_flag = 1
    ...
    @staticmethod
    def _ei(x, gp, y_max, xi, gp_model, gp_samples, meta_gps, w):
        if USE_DIRECT_OPTIMIZER:
            x = x.reshape(1, -1)
